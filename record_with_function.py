...
cash = initial_port_value - get_asset_value()


#Trading Record
def trading_record():
    price_diff = (get_last_price() - initial_price) / initial_price
    ini_hold_balance = (get_asset_value() + cash) + ((get_asset_value() * price_diff)) #initial_port_value + (initial_port_value * price_diff)
    ...
    print(f'Initial Price {initial_price}, Last Price {get_last_price()}')
    print(f'Initial Balance {initial_port_value} {rb_buffer}, Current Balance {get_current_balance()} {rb_asset}')
    print(f'Buy and hold : {ini_hold_balance} {rb_buffer}')
    print(f'Port Diff {port_diff * 100} % : Asset Price Diff {price_diff * 100} %')
    print(f'Cashflow is {cashflow} {rb_buffer}')
